...
function SortIcon({ column, activeColumn, activeDirection }: { column: SortKey; activeColumn: SortKey; activeDirection: "asc" | "desc" }) {
  if (activeColumn !== column) return <span className="text-gray-300 ml-1">&#8597;</span>;
  return <span className="text-gray-700 ml-1">{activeDirection === "asc" ? "↑" : "↓"}</span>;
}
...
function SortHeader({ column, activeColumn, activeDirection, onSort, children }: { column: SortKey; activeColumn: SortKey; activeDirection: "asc" | "desc"; onSort: (col: SortKey) => void; children: React.ReactNode }) {
  return (
    <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider select-none">
      <button type="button" onClick={() => onSort(column)} className="flex items-center w-full text-left hover:bg-gray-100 py-1 -my-1 px-1 -mx-1 rounded">
        {children}
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="animate-pulse space-y-3">
          ...
          {[1,2,3,4,5].map(n => <div key={n} className="h-14 bg-gray-200 rounded" />)}
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-16">
        <p className="text-red-600 mb-2">Failed to load ban list</p>
        <button onClick={() => refetch()} className="text-sm font-medium text-red-700 underline hover:no-underline">Retry</button>
        <br />
  ...
  return (
    <div className="space-y-6">
      ...
      {sortedEntries.length === 0 ? (
        <p className="text-gray-400">No ban list entries found.</p>
      ) : (
              ...
              {sortedEntries.map((entry) => (
                <tr key={entry.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm">
                    {entry.food ? (
                      <Link to={`/foods/${entry.food.id}`} className="font-medium capitalize text-blue-600 hover:underline">
                        {entry.food.name}
                      </Link>
                    ) : (
                      <span className="italic text-gray-400">Unknown food</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-500">
                    {entry.food?.category || <span className="text-gray-300">—</span>}
                  </td>
                  <td className="px-4 py-3 text-sm">
                    {entry.food?.health_index != null ? (
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                        entry.food.health_index >= 75 ? "bg-green-100 text-green-700" :
                        entry.food.health_index >= 50 ? "bg-yellow-100 text-yellow-700" :
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">
                    {entry.lethal_dose_mg != null ? `${entry.lethal_dose_mg} mg` : <span className="text-gray-300">—</span>}
                  </td>
                  <td className="px-4 py-3 text-sm">
                    {entry.is_conditionally_safe ? (
                      <span title={entry.safe_condition || "Conditionally safe"} className="inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full bg-yellow-100 text-yellow-700">
                        Conditional
