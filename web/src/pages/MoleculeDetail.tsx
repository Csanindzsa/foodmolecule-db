...
export default function MoleculeDetail() {
    ...
    return (
      <div className="space-y-8 animate-pulse">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[1,2,3,4].map(n => (
            <div key={n} className="h-16 bg-gray-200 rounded-xl" />
          ))}
        </div>
        <div className="h-10 bg-gray-200 rounded-lg w-1/3" />
        <div className="space-y-2">
          {[1,2,3].map(n => <div key={n} className="h-14 bg-gray-200 rounded-lg" />)}
        </div>
      </div>
    );
  }
  ...
  if (moleculeError) {
    return (
      <div className="text-center py-16">
        <p className="text-red-600 mb-2">Failed to load molecule details</p>
        <p className="text-sm text-gray-500">The molecule may not exist or has been removed.</p>
        <Link to="/" className="text-sm text-blue-600 underline mt-4 inline-block">Back to Home</Link>
      </div>
    );
  }
  ...
  const harmColor = molecule.harm_level >= 4 ? "bg-red-100 text-red-700" :
    molecule.harm_level >= 2 ? "bg-yellow-100 text-yellow-700" :
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        {molecule.iupac_name && (
          <p className="text-sm text-gray-500 mt-1">{molecule.iupac_name}</p>
        )}
      </div>
        ...
        <section>
          ...
          <ul className="space-y-2">
            {molecule.harm_mechanisms.map((mechanism, i) => (
              <li key={i} className="flex items-start gap-2 p-3 rounded-lg border bg-white">
      ...
      <section>
        ...
        {neutralizationsError ? (
          <div className="p-4 rounded-xl border bg-red-50 text-center">
            <p className="text-sm text-red-600 mb-2">Failed to load neutralization methods</p>
            <button onClick={() => refetchNeutralizations()} className="text-sm font-medium text-red-700 underline hover:no-underline">Retry</button>
          </div>
        ) : neutralizationsLoading ? (
          <div className="space-y-2">
            {[1,2].map(n => <div key={n} className="h-12 bg-gray-200 rounded-lg animate-pulse" />)}
          </div>
          ...
          <div className="space-y-2">
            {neutralizations.map((nm: unknown, i: number) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-lg border bg-white">
                ...
                <span className="text-sm text-gray-700">{nm != null && typeof nm === "object" ? String((nm as any).method ?? nm) : String(nm)}</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-gray-400 text-sm">No neutralization methods known.</p>
        )}
      </section>
      ...
      <section>
        ...
        {foodsError ? (
          <div className="p-4 rounded-xl border bg-red-50 text-center">
            <p className="text-sm text-red-600 mb-2">Failed to load foods</p>
            <button onClick={() => refetchFoods()} className="text-sm font-medium text-red-700 underline hover:no-underline">Retry</button>
          </div>
        ) : foodsLoading ? (
          <div className="space-y-2">
            {[1,2,3].map(n => (
              <div key={n} className="flex items-center p-3 rounded-lg border bg-white">
          ...
          <div className="space-y-2">
            {foods.map((food) => (
              <Link
                key={food.id}
                to={`/foods/${food.id}`}
                className="flex items-center justify-between p-3 rounded-lg border bg-white hover:shadow transition"
              >
                <div>
                  <span className="font-medium capitalize">{food.name}</span>
                  {food.category && <span className="text-xs text-gray-400 ml-2">{food.category}</span>}
                </div>
          ...
          <p className="text-gray-400 text-sm">No food data available.</p>
        )}
      </section>
    </div>
  );
