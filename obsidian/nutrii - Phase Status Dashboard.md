...
| Seed data (foods) | 🟡 Partial | 5,003 USDA-derived seed JSON files are present and covered by schema/readiness checks plus CI pipeline dry-run; production DB load still required |
...
**Notes:** All pipeline infrastructure is built. Food seed files are present at launch-baseline scale and validated through CI schema checks, seed readiness, and ingestion dry-run; production database loading remains a launch task. Molecule/study coverage still needs expansion.
...
| History store (`useHistoryStore.ts`) | ✅ Complete | Zustand + AsyncStorage with five-item scan history |
...
**Notes:** API-backed mobile flows now cover food and molecule search, compare, rich food and molecule detail, OCR scanning, draft ban-list review, and recent scan history. Food detail shares enriched image, molecule, health-index breakdown, AI guide, and PubMed research context with the backend. Native build, physical-device permission testing, and real-label OCR validation remain open.
