...
- `web/src/App.tsx` defines the launch routes: `/`, `/search`, `/foods/:id`, `/molecules/:id`, `/compare`, and `/ban-list`.
- `web/public/sitemap.xml` contains the static crawlable routes: `/`, `/search`, `/compare`, and `/ban-list`.
- `web/src/components/Layout.tsx` exposes Compare and Ban List links in desktop and mobile header navigation.
