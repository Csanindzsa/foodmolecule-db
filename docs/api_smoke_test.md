...
The command exits non-zero if any selected probe returns an unexpected status or cannot connect within the timeout.
...
`--require-full` fails fast unless all 17 public API routes are represented in the selected probe set. `food-guide` accepts `200` or `404`, because a seeded food can exist before its AI guide is generated. The OCR scan probe expects the production scan endpoint to return `200`; a `503`, `415`, or `422` should be treated as a launch issue unless the input file is invalid.
